...
![](./../note_img/ControlNet/20240717_164657.png)
...
![](./../note_img/ControlNet/20240717_165216.png)
...
![](./../note_img/ControlNet/20240717_173526.png)
...
#### **M02 ：生成效果作为参考图生成一张二次元的图，要求保持微笑，穿着旗袍，露出手臂。**

1. 将生成效果图导入 ControlNet 选择 Tile 模型，down sample rate 提到 4，选择 **偏向提示词模式**。

   这里选择偏向提示词模式有两个原因：
   ①是因为我们不要参考图的机甲，所以必须选择 **偏向提示词模式**；
   ②是这一步需要完成转换风格，偏向提示词也是更好的选择。

> **Down Sample Rate**：可以理解为参考图缩小倍数。假设参考图分辨率为 768：
>
> - Down sample rate 为 1，预处理图分辨率为 768
>- Down sample rate 为 2，预处理图分辨率为 384（768的 1/2）
> - Down sample rate 为 8，预处理图分辨率为 96（768的 1/8）

> **为什么要降低预处理图的分辨率？**
>
> Tile模型的特点是将图片切分成小块，然后将每个小块的内容与提示词匹配。降低分辨率是为了让Tile有足够的发挥空间，消除参考图的细节，重构新图像的细节，以达到修改细节的目标。

2. 切换到二次元的模型，在提示词中添加旗袍、赤裸的手臂、微笑等提示词，生成出来的图我们发现她的手臂还是有可能会有手套，为了防止ControlNet将参考图中的手识别成手套，应该在反向提示词中补充手套等提示词，最终得到的效果。
...
![](./../note_img/Controlnet/20240717_212626.png)
...
![](./../note_img/Controlnet/20240717_215306.png)
...
![](./../note_img/Controlnet/20240717_225627.png)
...
![](./../note_img/Controlnet/20240718_095626.png)
