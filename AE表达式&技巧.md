...
```javascript
// 让图层对蒙版做出反应
// 新建"纯色"作为遮罩并命名"MASK"，遮罩蒙版命名为"蒙版 1"
...
const maskName = "蒙版 1"; // Change this to the name of your mask
...
var numDivisions = 5; 
var maskLayer = thisComp.layer("MASK"); 
var maskName = "蒙版 1"; 
var maskPath = maskLayer.mask(maskName).maskPath;
var rawMaskPoints = maskPath.points();
var inTangents = maskPath.inTangents();
var outTangents = maskPath.outTangents();
var isMaskClosed = maskPath.isClosed();
var maskFeather = maskLayer.mask(maskName).maskFeather[0];
var fallOffSquared = Math.pow(maskFeather, 2);

function needsSubdivision(c1, c2) {
    var tangentThreshold = 0.1;
    return (length(c1) > tangentThreshold || length(c2) > tangentThreshold);
}
...
function transformMaskPoints(layer, pathPoints, inTangents, outTangents, isClosed) {
    ...
    for (var i = 0; i < count; i++) {
        var nextIndex = (i + 1) % pathPoints.length;
        var c1 = [pathPoints[i][0] + outTangents[i][0], pathPoints[i][1] + outTangents[i][1]];
        var c2 = [pathPoints[nextIndex][0] + inTangents[nextIndex][0], pathPoints[nextIndex][1] + inTangents[nextIndex][1]];
        if (needsSubdivision(outTangents[i], inTangents[nextIndex])) {
            allPoints = allPoints.concat(subdivideBezierSegment(pathPoints[i], c1, c2, pathPoints[nextIndex], numDivisions));
        } else {
            allPoints.push(pathPoints[i], pathPoints[nextIndex]);
    ...
    for (var j = 0; j < allPoints.length; j++) {
        transformedPoints.push(layer.toComp(allPoints[j]));
    }
    return transformedPoints;
}

var transformedMaskPoints = transformMaskPoints(maskLayer, rawMaskPoints, inTangents, outTangents, isMaskClosed);
...
function distanceToLineSquared(point, a, b) {
    var lineLengthSquared = Math.pow(a[0] - b[0], 2) + Math.pow(a[1] - b[1], 2);
    if (lineLengthSquared == 0) return Math.pow(point[0] - a[0], 2) + Math.pow(point[1] - a[1], 2);
    var t = ((point[0] - a[0]) * (b[0] - a[0]) + (point[1] - a[1]) * (b[1] - a[1])) / lineLengthSquared;
    t = Math.max(0, Math.min(1, t));
    return Math.pow(point[0] - (a[0] + t * (b[0] - a[0])), 2) + Math.pow(point[1] - (a[1] + t * (b[1] - a[1])), 2);
}

function closestDistanceSquared(point, path) {
    var closestDistSquared = Infinity;
    for (var i = 0; i < path.length - 1; i++) {
        var distSquared = distanceToLineSquared(point, path[i], path[i + 1]);
        if (distSquared < closestDistSquared) closestDistSquared = distSquared;
    }
    return closestDistSquared;
}

var anchorPoint = thisLayer.transform.anchorPoint;
var toCompAnchor = thisLayer.toComp([anchorPoint[0], anchorPoint[1]]);
var effectValue = thisProperty.key(1).value;
var closestDistSquared = closestDistanceSquared(toCompAnchor, transformedMaskPoints);
if (isMaskClosed) {
    var isInside = inside(toCompAnchor, transformedMaskPoints);
    if (isInside) {
        effectValue = thisProperty.key(2).value;
    } else if (maskFeather > 0 && closestDistSquared <= fallOffSquared) {
        var normalizedDistance = Math.sqrt(closestDistSquared) / maskFeather;
        effectValue = linear(normalizedDistance, 0, 1, thisProperty.key(2).value, thisProperty.key(1).value);
    }
} else {
    if (maskFeather > 0 && closestDistSquared <= fallOffSquared) {
