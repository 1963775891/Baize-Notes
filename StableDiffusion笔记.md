...
```markdown
1.请详细分析下面的图像，从画面内容：主体特征，主体描述，背景风格描述，风格：艺术风格，色，材质类型，设计师，构图，设计风格，画面质量，细节，属性词：情绪，打光，镜头，角度，渲染工具，美学等进行分析，输出分析的英文prompt。
2.生成一个同设计风格，不包含主体内容的通用的stable diffusion使用的英文词组式prompt。
```
