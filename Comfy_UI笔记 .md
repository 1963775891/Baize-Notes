...
`-`	 **减法**：从一个数字中减去另一个数字。例如，`5 - 2 = 3`。

`*`	**乘法**：将两个数字相乘。例如，`3 * 2 = 6`。

`/` 	**除法**：将一个数字除以另一个数字。例如，`6 / 2 = 3`。

`//` 	**整除**（地板除法）：返回除法后的整数部分，舍弃小数部分。例如，`7 // 2 = 3`。

`**`	 **乘方**：计算一个数的幂。例如，`3 ** 2 = 9`表示3的平方。

`^` 	**按位异或**：对两个数字进行按位异或操作（仅对整数有效）。例如，`5 ^ 3 = 6`。

`%` 	**取模**：返回除法后的余数。例如，`7 % 2 = 1`。

**支持的函数：**

```markdown
- floor(num, dp?) 
向下取整：将数字向下取整，`dp`可选参数表示保留小数位数。如果不指定`dp`，则直接向下取整到最接近的整数。
   - 示例：`floor(5.7)` 返回 5。
   - 示例：`floor(5.768, 1)` 返回 5.7。
...
- randomint(min,max)
生成随机整数：在指定的最小值`min`和最大值`max`之间生成一个随机整数。
   - 示例：`randomint(1, 10)` 可能返回任何在1到10之间的整数。
```
...
1. 如果设置分区域条件：**第一个条件，只写质量词**，将其他的条件详细描写。
...
​	**注意：第一个条件一定是全体的，不然就会以设置第一个条件的区域绘图！！！**

2. **偏向Controlnet**：将不添加描述词的Controlnet放在第一条件，将详细描述的放在第二，
3. **多Controlnet**：将主要的Controlnet第一条件

![](./note_img/Controlnet/20240824_101417.jpg)

> **条件合并：**将所有的条件合并在一起，所以当提示词重复和相同的，就会叠加强度；
